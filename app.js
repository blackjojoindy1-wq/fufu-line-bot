// LINE Booking Bot – Final Fix (Confirm works + Multilingual reg card)
// -------------------------------------------------------------------
// Fixes for "stuck on Confirm":
// 1) Confirmation card footer is VERTICAL (more reliable across LINE clients).
// 2) Postback uses robust query format: action=confirm&value=YES (and we also accept confirm=YES).
// 3) Added displayText on buttons (some clients rely on it to emit postback).
// 4) Safer postback parser with URLSearchParams + fallback.
// 5) After confirmation: send received note + plain-text summary + multilingual branch-specific registration card.
// 6) Starts ONLY on keywords: "book" / "จอง" / "予約". One-time hint on follow. No spam after completion.
...
function showMenuForBranch(lang, branch) { const url = branch === 'THONGLOR' ? MENUS.THONGLOR : MENUS.PHROMPHONG; return { type: 'text', text: `${t(lang, 'SHOW_MENU')}\n${url}` }; }
...
function confirmationFlex(lang, s) {
  ...
  return { type: 'flex', altText: t(lang, 'CONFIRM_TITLE'), contents: { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: t(lang, 'CONFIRM_TITLE'), weight: 'bold', size: 'md' }] }, body: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [ { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Branch', flex: 2 }, { type: 'text', text: branchLabel, flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Date', flex: 2 }, { type: 'text', text: s.date || '-', flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Time', flex: 2 }, { type: 'text', text: s.time || '-', flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Service', flex: 2 }, { type: 'text', text: s.service || '-', flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Name', flex: 2 }, { type: 'text', text: s.name || '-', flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Phone', flex: 2 }, { type: 'text', text: s.phone || '-', flex: 5 }] }, { type: 'box', layout: 'baseline', contents: [{ type: 'text', text: 'Discount', flex: 2 }, { type: 'text', text: s.discount || '-', flex: 5 }] } ] }, footer: { type: 'box', layout: 'vertical', spacing: 'md', contents: [ { type: 'button', style: 'primary', action: { type: 'postback', label: t(lang, 'BTN_CONFIRM'), data: 'action=confirm&value=YES', displayText: t(lang, 'DISPLAY_CONFIRM') } }, { type: 'button', style: 'secondary', action: { type: 'postback', label: t(lang, 'BTN_EDIT'), data: 'action=confirm&value=EDIT', displayText: t(lang, 'DISPLAY_EDIT') } } ] } } };
}
...
async function handleEvent(event) {
  ...
  if (event.type === 'follow') {
    // One-time hint only when user adds the bot
  ...
  if (event.type === 'message' && event.message.type === 'text') {
    ...
    // If nothing started, stay quiet (no spam)
    ...
    // After completion, do nothing on random messages
