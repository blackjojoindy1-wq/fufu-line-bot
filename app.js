// LINE Booking Bot – Triggered Flow (Registration Card + Branch-specific Button)
// -----------------------------------------------------------------------------
// ✅ Starts ONLY when user types one of: "book" (EN), "จอง" (TH), or "予約" (JP)
// ✅ Sends the "how to book" hint ONLY on first add (follow event)
// ✅ After booking confirmation, sends: received note + plain-text summary +
//    a **Flex registration card** with a clean button to the correct branch form
...
// Branch-specific registration forms
const REG_FORMS = {
...
function registrationCard(branch) {
  const url = branch === 'THONGLOR' ? REG_FORMS.THONGLOR : REG_FORMS.PHROMPHONG;
  const branchLabel = branch === 'THONGLOR' ? 'Thong Lo' : 'Phrom Phong';
  return {
    type: 'flex',
    altText: 'New Customer Registration',
    contents: {
      type: 'bubble',
      header: {
        type: 'box',
        layout: 'vertical',
        contents: [
          { type: 'text', text: 'New Customer Registration', weight: 'bold', size: 'lg' },
      ...
      body: {
        ...
        spacing: 'md',
        contents: [
          { type: 'text', text: 'PLease fill in if New Customer.' },
        ],
      },
      footer: {
        type: 'box',
        layout: 'vertical',
        contents: [
          { type: 'button', style: 'primary', action: { type: 'uri', label: 'Open Registration Form', uri: url } },
        ],
      },
    },
  };
    ...
    if (data.confirm) {
      if (data.confirm === 'YES') {
        ...
        const msgs = [
          ...
          registrationCard(state.branch || 'THONGLOR'),
        ];
